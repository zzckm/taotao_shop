...
var category = {OBJ: $("#_JD_ALLSORT"),
        ...
        return "" == c ? d : (/^http[s]?:\/\/([\w-]+\.)+[\w-]+([\w-.\/?%&=]*)?$/.test(c) || (c = c.replace(/-000$/, ""), c = c.match(/^\d*-\d*$/) ? "http://channel.jd.com/" + c + ".html" : "http://localhost:8082" + c), '<a href="' + c + '">' + d + "</a>")
       ...
       /* var a, b = {}, c = $("#categorys-2013");
    	...
    	//使用jsonp来实现跨域请求(跨url请求数据显示商品类目)    	
        $.getJSONP(this.URL_Serv/*, category.getDataService*/);
        
    	//直接使用ajax请求json数据（显示商品类目）
